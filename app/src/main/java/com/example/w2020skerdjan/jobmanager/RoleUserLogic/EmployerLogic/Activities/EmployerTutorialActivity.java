...
public class EmployerTutorialActivity extends AppCompatActivity {
    ...
    private String name, last_name,mobile_number ,education, profession, experience_years, company;
    ...
    private boolean validate(){
        ...
        if(company == null || company.length()==0){
            if(flag = true)
                Toast.makeText(this, "Please insert a Company!", Toast.LENGTH_SHORT).show();
            flag=false;
        }
        ...
        if(profession == null || profession.length()==0 || profession.equals("Profession") || pagerAdapter.getFragmentByPosition(3).returnIdFromProfession(profession)==-1){
            if(flag = true)
                Log.d("Skerdi " , "Profession : " + profession);
    ...
    private void endTutorial(){
        if(member!=null){
            Log.d("Skerdi", "End tutorial success");
           ...
           requestsAPI.updateMemberWithId(member.getMemberID(), company, member).enqueue(updateMemberCallback);
        }
        else{
